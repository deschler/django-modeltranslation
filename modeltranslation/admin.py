...
class TranslationAdmin(TranslationBaseModelAdmin[_ModelT], admin.ModelAdmin[_ModelT]):
    ...
    def get_form(
        ...
        return self._patch_validation_exclusions(super().get_form(request, obj, **kwargs))

    def _patch_validation_exclusions(self, form: type[forms.ModelForm]):
        """
        If unique validation uses translated fields, we need to preserve them for
        errors to be raised correctly during form handing.

        Refs https://github.com/deschler/django-modeltranslation/issues/261
        """

        translated_fields = self.trans_opts.fields
        original = form._get_validation_exclusions

        def _get_validation_exclusions(self) -> set[str]:
            exclusions = original(self)
            # Keep translated original fields in validation
            return exclusions.difference(translated_fields)

        form._get_validation_exclusions = _get_validation_exclusions
